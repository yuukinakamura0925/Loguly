      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-700 bg-slate-800/80">
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3 py-1.5 bg-amber-500/20 text-amber-400 text-xs font-medium rounded hover:bg-amber-500/30 transition-colors border border-amber-500/30"
          >
            + 字幕
          </button>
          <button
            onClick={onAddAudio}
            className="px-3 py-1.5 bg-teal-500/20 text-teal-400 text-xs font-medium rounded hover:bg-teal-500/30 transition-colors border border-teal-500/30"
          >
            + 読み上げ
          </button>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-slate-400 font-mono w-8 text-center">{zoom}x</span>
            <button
      ...
      <DndContext sensors={sensors} onDragMove={handleDragMove} onDragEnd={handleDragEnd}>
        <div className="flex flex-1 min-h-0">
          ...
          >
            ...
            {/* Video track label */}
            <div className="h-12 flex items-center px-2 border-b border-slate-700/50">
              <span className="text-xs text-slate-400 font-medium">動画</span>
            </div>
            {/* Subtitle track label */}
            <div className="h-14 flex items-center px-2 border-b border-slate-700/50">
              <span className="text-xs text-amber-400 font-medium">字幕</span>
            </div>
            {/* Audio track label */}
            <div className="h-14 flex items-center px-2">
              <span className="text-xs text-teal-400 font-medium">読み上げ</span>
            </div>
          </div>
          ...
          >
            ...
            <div className="relative" style={{ width: `${zoom * 100}%`, minWidth: '100%' }}>
              {/* Time Ruler */}
              <div className="h-8 border-b border-slate-700 relative bg-slate-900 overflow-hidden">
                {markers.map((time, index) => {
                  ...
                  return (
                    ...
                    >
                      {/* 縦線は正確な位置 */}
                      <div className="absolute bottom-0 left-0 h-3 w-px bg-slate-600 -translate-x-1/2" />
                      {/* テキストは端でははみ出さないように調整 */}
                      <span
                        className="text-[11px] text-slate-500 whitespace-nowrap mb-3"
                        style={{
              ...
              {/* Video Track */}
              <div
                className="h-12 border-b border-slate-700/50 relative px-1 flex items-center cursor-pointer"
                onClick={handleTrackClick}
              >
                <div className="h-9 w-full bg-blue-600/30 border border-blue-500/50 rounded" />
              </div>

              {/* Subtitle Track */}
              <div
                className="h-14 border-b border-slate-700/50 relative flex items-center cursor-pointer"
                onClick={handleTrackClick}
              >
              ...
              {/* Audio Track */}
              <div
                className="h-14 relative flex items-center cursor-pointer bg-slate-800/50"
                onClick={handleTrackClick}
              >
